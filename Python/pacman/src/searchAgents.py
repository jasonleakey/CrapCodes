...
# -*- coding: utf-8 -*-
#
#  Nov 16, 2009 - search agents and problem definitions
...
class SearchAgent(Agent):
  ...
  def getAction(self, state):
    ...
    "*** YOUR CODE HERE ***"
    curAction = self.actions[self.__class__.INDEX]
...
def foodHeuristic(state):
  ...
  "*** YOUR CODE HERE ***"
  x, y = state[0]
#  cnt = -sum([c[max(y - 3, 0) : min(y + 4, state[1].height)].count(True)\
#                for c in state[1][max(x - 3, 0) : min(x + 4, state[1].width)]])
#  print cnt
  foodPosList = state[1].asList()
  minDist = 0x7FFFFFFF
  for item in foodPosList:
      dis = util.manhattanDistance(item, (x, y))
      if dis < minDist:
          minDist = dis
  return minDist
#  return state[1].count()
